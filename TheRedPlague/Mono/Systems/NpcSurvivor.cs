...
{
    private const float MinInsanity = 10f;
    
    public ModelType model;
    public string survivorName;
    public string requiredStoryGoal;
    public string deathStoryGoal;
    public float minInterval = 3 * 60;
    public float maxInterval = 6 * 60;
    ...
    private bool IsReadyToSpawn()
    {
        ...
        if (InsanityManager.Main && InsanityManager.Main.Insanity < MinInsanity)
            return false;
    ...
    private bool TryGetValidPath()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            if (GenericTrpUtils.IsPositionOnScreen(pos) && !Physics.SphereCast(new Ray(pos, (inFrontOfCameraPos - pos).normalized), 1, (inFrontOfCameraPos - pos).magnitude, -1, QueryTriggerInteraction.Ignore))
            {
                midPos = pos;
                foundMidPos = true;
                break;
